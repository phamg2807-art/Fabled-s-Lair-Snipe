...
biome_counts = {}
merchant_counts = {}  # Track all merchant spawns
...
class RenderHealthCheckHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        html += """
                ...
                <h2>🛒 Merchant Sniper Counters</h2>
                <div class="card">
        """
        if not metrics['merchant_counts']:
            html += '<p class="empty">No merchants tracked yet during this active runtime session.</p>'
        else:
            html += '<div class="biome-grid">'
            for m_name, count in sorted(metrics['merchant_counts'].items()):
                html += f"""
                <div class="biome-item" style="border-color: #f59e0b;">
                    <div class="biome-name" style="color: #f59e0b;">{m_name}</div>
                    <div class="biome-num">{count}</div>
                </div>
                """
            html += '</div>'
...
async def on_message(message):
    ...
    # Track every single incoming webhook packet to count unique streams and assess activity window
    ...
    for embed in message.embeds:
        ...
        # Expanded keyword array to support merchant behaviors (spawned, left, arrived)
        ...
        # GATING ROUTE: Process as Merchant or Biome
        if "merchant" in combined_text.lower():
            ...
            print(f"📈 Total {merchant_name}s: {merchant_counts[merchant_name]}")
            ...
            if biome_match:
                ...
                # Prioritize high-rarity sub-biomes over generic weather elements
